...
import com.sth.jvm.util.CmdLineOption;
...
public class CoreAgent {
    ...
    public static void premain(String args, Instrumentation inst) {
        ...
        options.addOption("debug", true, "调试模式");
        options.addOption("del", true, "读取密码后删除密码");

        char[] pwd;
        String jarFilePath=JarUtils.getRootPath(null);

        //读取jar隐藏的密码，无密码启动模式(jar)
        System.out.println("path="+jarFilePath);
        pwd = JarDecryptor.readPassFromJar(new File(jarFilePath));
        ...
        if (StrUtils.isEmpty(pwd)) {
            Log.debug("参数、环境变量都没密码，读取密码配置文件");
        ...
        //GO
        if (inst != null) {
            AgentTransformer tran = new AgentTransformer(pwd,jarFilePath);
